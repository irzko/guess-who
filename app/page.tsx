...
export default function Home() {
  async function createSeed(formData: FormData) {
    ...
    redirect(
      "/pick/" + encodeURIComponent(Buffer.from(seed).toString("base64")),
    );
  }
  return (
    <div className="max-w-screen-lg relative w-full p-2">
      <div className="h-full flex flex-col items-center justify-center w-full">
        ...
        >
          ...
          />
          <button className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-bold rounded-xl w-full sm:w-auto px-5 py-2.5 text-center">
            Bắt đầu
          </button>
        </form>
      </div>
